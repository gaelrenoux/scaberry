...
import scaberry.oldmacros.checks.Checkers
  ...
  protected def fieldToValue(sField: TermSymbol, srcTpe: Type, copyParams: List[TermSymbol]): (Tree, Tree) = {
    ...
    val field =
      if (copyable) {
        ...
        q"new scaberry.core.CopyableField[$srcTpe, $typ]($nameAsSymbol, _.$name, $copier)"
      } else {
        q"new scaberry.core.Field[$srcTpe, $typ]($nameAsSymbol, _.$name)"
      }

...
import scaberry.oldtests.data.{Animal, Dog}
...
class FieldsMacroSpec extends FlatSpec with Matchers with Helpers {
  ...
  def f: (Field[Any, Any], CopyableField[Any, Any]) = ???
  ...
  they should "carry the correct name" in {
    ...
    force.arf(data.Dog.publicFields.name).name.name should be("name")
    force.arf(data.Dog.publicFields.weight).name.name should be("weight")
    force.arf(data.Dog.publicFields.owner).name.name should be("owner")
    force.arf(data.Dog.publicFields.otherName).name.name should be("otherName")
    force.arf(data.Dog.publicFields.otherOtherName).name.name should be("otherOtherName")
    force.arf(data.Dog.constructorFields.name).name.name should be("name")
    force.arf(data.Dog.constructorFields.weight).name.name should be("weight")
  }
  ...
  they should "carry the correct class tags" in {
    ...
    force.arf(data.Dog.publicFields.name).typeClassTag should be(implicitly[ClassTag[Some[String]]])
    force.arf(data.Dog.publicFields.weight).typeClassTag should be(implicitly[ClassTag[Long]])
    force.arf(data.Dog.publicFields.owner).typeClassTag should be(implicitly[ClassTag[String]])
    force.arf(data.Dog.publicFields.otherName).typeClassTag should be(implicitly[ClassTag[Option[String]]])
    force.arf(data.Dog.publicFields.otherOtherName).typeClassTag should be(implicitly[ClassTag[Option[String]]])
    force.arf(data.Dog.constructorFields.name).typeClassTag should be(implicitly[ClassTag[Some[String]]])
    force.arf(data.Dog.constructorFields.weight).typeClassTag should be(implicitly[ClassTag[Long]])
  }
  ...
  "Getters" should "return the correct value" in {
    ...
    force.arf(data.Dog.publicFields.name)(rex).get should be(Some("Rex"))
    force.arf(data.Dog.publicFields.weight)(rex).get should be(9L)
    force.arf(data.Dog.publicFields.owner)(rex).get should be("Unknown")
    force.arf(data.Dog.publicFields.otherName)(rex).get should be(Some("Rex"))
    force.arf(data.Dog.publicFields.otherOtherName)(rex).get should be(Some("Rex"))
    force.arf(data.Dog.constructorFields.name)(rex).get should be(Some("Rex"))
    force.arf(data.Dog.constructorFields.weight)(rex).get should be(9L)
  }

  "Copiers" should "return the correct value" in {
    force.dcf[Some[String]](data.Dog.publicFields.name)(rex).copy(Some("Medor")) should be(rex.copy(name = Some("Medor")))
    force.dcf[Long](data.Dog.publicFields.weight)(rex).copy(12L) should be(rex.copy(weight = 12L))
    force.dcf[Some[String]](data.Dog.constructorFields.name)(rex).copy(Some("Medor")) should be(rex.copy(name = Some("Medor")))
    force.dcf[Long](data.Dog.constructorFields.weight)(rex).copy(12L) should be(rex.copy(weight = 12L))
  }

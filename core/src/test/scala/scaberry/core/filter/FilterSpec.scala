...
class FilterSpec extends FlatSpec with Matchers {
  ...
  val nameFilter: Filter.Field[Person] = Filter.field(Person.Fields.name, Filter.value("Charles Darwin"))
  val ageFilter: Filter.Field[Person] = Filter.field(Person.Fields.age, Filter.value(42L))
